...
INT32 AsicAutoFallbackInit(RTMP_ADAPTER *pAd)
{
	...
	Value |= RATE2_DOWN_MPDU_LIMIT(1);
	Value &= ~RATE3_DOWN_MPDU_LIMIT_MASK;
	Value |= RATE3_DOWN_MPDU_LIMIT(1);
	Value &= ~RATE4_DOWN_MPDU_LIMIT_MASK;
	Value |= RATE4_DOWN_MPDU_LIMIT(1);
	Value &= ~RATE5_DOWN_MPDU_LIMIT_MASK;
	Value |= RATE5_DOWN_MPDU_LIMIT(1);
	Value &= ~RATE6_DOWN_MPDU_LIMIT_MASK;
	Value |= RATE6_DOWN_MPDU_LIMIT(1);
	Value &= ~RATE7_DOWN_MPDU_LIMIT_MASK;
	Value |= RATE7_DOWN_MPDU_LIMIT(1);
	Value &= ~RATE8_DOWN_MPDU_LIMIT_MASK;
	Value |= RATE8_DOWN_MPDU_LIMIT(1);
...
{
	UINT32 tmac_tcr, agg_pcr, TxOpValue, tmac_trcr;
	...
	IN UCHAR channel)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: SlotTime =%ld us, SIFS Time = %ld us,\
							RIFS Time = %ld, EIFS Time = %ld\n",\
...
VOID AsicUpdateRxWCIDTable(RTMP_ADAPTER *pAd, USHORT WCID, UCHAR *pAddr)
{
	...
	dw0->field.muar_idx = 0xe;
        ...
        dw0->field.rv = 1;
        DBGPRINT(RT_DEBUG_TRACE, ("%s(): mac_entry->type=%d, pMacEntry->MaxHTPhyMode.field.MODE=%d, StatusFlag=0x%x\n",
...
INT AsicStartContinousTx(RTMP_ADAPTER *pAd, UINT32 PhyMode, UINT32 BW, UINT32 PriCh, UINT32 Mcs, UINT32 WFSel)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,("%s(): gc[0x0204]: 0x%x\n",__FUNCTION__, gc));
	/* End of GC Swap */
...
#define THERMCR0 0x80022000
VOID AsicGetThermSensorTemperature(RTMP_ADAPTER *pAd, INT32 *temp)
{
	UINT32 value,flag;
	INT32 y,b,s;
	/*temperature = (y-b)*(56+s)/30+offset*/

	RTMP_IO_READ32(pAd,THERMCR0,&value);

	y = (value >> 8) & 0x7f;

	flag = (pAd->EEPROMImage[0x55] >> 7) & 0x1;

	if(flag)
	{
		value = pAd->EEPROMImage[0x55] & 0x7f;
	...
	flag = (pAd->EEPROMImage[0x53] >> 7) & 0x1;

	if(flag)
	{
		value = pAd->EEPROMImage[0x54] & 0x1f;
	}else
	{
		value = 0;
	}
