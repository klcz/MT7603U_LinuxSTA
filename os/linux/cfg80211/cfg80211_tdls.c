	...
	VOID						*pAdOrg)
{
	...
	pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.IneedKey = 1;
	return TRUE;
}
		...
		if (pMacEntry)
		{
			...
			{	
				...
				if ( pMacEntry->Aid > 1)
				{
					...
					NdisMoveMemory(PairwiseKey.Key, &(pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TPK[LEN_TK]), LEN_TK);
...
void cfg_tdls_auto_teardown(PRTMP_ADAPTER pAd,UCHAR *peerAddr)
{
	...
	mdelay(1);
	cfg80211_tdls_oper_request(pAd->net_dev, peerAddr, NL80211_TDLS_DISABLE_LINK, 0, GFP_KERNEL);
#else
