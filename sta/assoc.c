	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	USHORT Status;
	/* for judge bss coex */
#ifdef BSS_COEX_DISABLE
	UCHAR bBssCoexEnable = TRUE;
#endif /* BSS_COEX_DISABLE */
...
#ifdef HT40_DISABLE_SHORT_GI
			/* Special requirement:
			HT20 Long GI : Enable
			HT20 Short GI: Enable
			HT40 Long GI : Disable
			HT40 Short GI: Disable */
#ifdef HE_BD_CFG80211_SUPPORT
		if (pAd->StaCfg.bHt40Test == FALSE) /* 0: not for ht40 tset now */
#endif /* HE_BD_CFG80211_SUPPORT */
		{
			//	if (pAd->CommonCfg.RegTransmitSetting.field.ShortGI == GI_800)
				if (pAd->CommonCfg.RegTransmitSetting.field.BW == BW_40)
				{
					DBGPRINT(RT_DEBUG_TRACE, ("#####%s()-BW_40, old HtCapInfo:%x\n",__FUNCTION__, pHtCapability->HtCapInfo));
					//pHtCapability->HtCapInfo.ShortGIfor20 = 0;
					pHtCapability->HtCapInfo.ShortGIfor40 = 0;
					DBGPRINT(RT_DEBUG_TRACE, ("#####%s()-BW_40, New HtCapInfo:%x\n",__FUNCTION__, pHtCapability->HtCapInfo));
				}
		}
#endif  /* HT40_DISABLE_SHORT_GI */
#ifdef BSS_COEX_DISABLE
#ifdef HE_BD_CFG80211_SUPPORT
			if (pAd->StaCfg.bHt40Test == FALSE) /* 0: not for ht40 tset now */
#endif /* HE_BD_CFG80211_SUPPORT */
			{
				if (pHtCapability->HtCapInfo.ChannelWidth == BW_20)
				{
					/* is not required for 20MHz STA in 2.4GHz */
					bBssCoexEnable = FALSE;
					/* disable this bit to avoid Update2040CoexistFrameAndNotify() */
					pAd->CommonCfg.ExtCapIE.BssCoexistMgmtSupport = 0;

					DBGPRINT(RT_DEBUG_TRACE, ("#####%s()-BW_40, old HtCapInfo:%x\n",__FUNCTION__, pHtCapability->HtCapInfo.ChannelWidth));
				}
			}
#endif /* BSS_COEX_DISABLE */
