	...
	IN	RTMP_STRING *pInput)
{
    ...
    if (RTMPGetKeyParameter("HT_BAWinSize", pValueStr, 25, pInput, TRUE))
	{
		...
		if (Value >= 1 && Value <= 64)
		{		
			pAd->CommonCfg.REGBACapability.field.RxBAWinLimit = min(Value, pChipCap->RxBAWinSize);
			pAd->CommonCfg.BACapability.field.RxBAWinLimit = min(Value, pChipCap->RxBAWinSize);
			DBGPRINT(RT_DEBUG_TRACE, ("HT: BA Windw Size = %d\n", min(Value, pChipCap->RxBAWinSize)));
		}
		else
		{
            pAd->CommonCfg.REGBACapability.field.RxBAWinLimit = min(64, pChipCap->RxBAWinSize);
			pAd->CommonCfg.BACapability.field.RxBAWinLimit = min(64, pChipCap->RxBAWinSize);
			DBGPRINT(RT_DEBUG_TRACE, ("HT: BA Windw Size = %d\n", min(64, pChipCap->RxBAWinSize)));
		}
	...
	IN RTMP_STRING *pBuffer)
{
...
#ifdef DFS_SUPPORT
	{
      ...
      {   
         ...
         DBGPRINT(RT_DEBUG_OFF, ("PS_RETRIEVE = %x\n",PS_RETRIEVE));
      }
